    ...
    K: SincKind,
{
        ...
        M: bittle::Mask,
    {
        ...
        for mut wav in self.buffer.iter_mut() {
        ...
        for (out, wave_in) in mask.join(self.buffer.iter_mut().zip(wave_in.iter())) {
...
mod tests {
    ...
    fn make_resampler_fi_skipped() {
        ...
        let mask: bittle::BitSet<u128> = bittle::bit_set![1];
        let out = resampler.process(&waves, &mask).unwrap();
        assert_eq!(out.len(), 2);
    ...
    fn test_boxed_resampler() {
        ...
        let frames = resampler.nbr_frames_needed();
        let waves = audio::dynamic![[0.0f64; frames]; 2];
        let _ = resampler.process(&waves, &bittle::bit_set![]).unwrap();
    }
}
