    ...
    T: Sample,
{
    ...
    fn resample_unit(
        ...
        audio::channel::copy(
            inp,
            audio::channel::LinearMut::new(&mut self.input_buf[..self.fft_size_in]),
        );
    ...
    T: Sample,
{
        ...
        M: bittle::Mask,
    {
        ...
        for (chan, wave) in mask.join(wave_in.iter().enumerate()) {
    ...
    T: Sample,
{
        ...
        M: bittle::Mask,
    {
        ...
        if processed_frames > self.chunk_size_out {
            for (wave_out, out) in mask.join(wave_out.iter().zip(self.output_buffers.iter_mut())) {
                audio::channel::copy(
    ...
    T: Sample,
{
        ...
        M: bittle::Mask,
    {
        ...
        if self.saved_frames > frames_in_used {
            let it = input_temp.iter().zip(self.input_buffers.iter_mut());
...
mod tests {
    ...
    fn resample_unit() {
        ...
        resampler.resample_unit(
            &mut overlap,
            audio::channel::LinearRef::new(&wave_in[..]),
            audio::channel::LinearMut::new(&mut wave_out[..]),
        );
    ...
    fn make_resampler_fio_skipped() {
        ...
        let mask: bittle::BitSet<u128> = bittle::bit_set![0];
